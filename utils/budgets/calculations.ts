...
export const calculateChartPercentages = (metrics: any) => {
	...
	const clientsWithBudgetPercentage = calculatePercentage(
		metrics.clientsWithBudget,
				...
				data:
					metrics.totalBudgets > 0
						? [
								...
								{
									name: 'En proceso',
									value: metrics.totalBudgets - (metrics.totalSales + metrics.totalLost),
									color: '#3b82f6',
								},
		...
		charts: [
			{
				...
				data:
					metrics.budgetsByMaterial && metrics.budgetsByMaterial.length > 0
						? getPercentages(metrics.budgetsByMaterial, metrics.totalBudgets).map(
			...
			},
			{
				...
				data:
					metrics.soldBudgetsByMaterial && metrics.soldBudgetsByMaterial.length > 0
						? getPercentages(metrics.soldBudgetsByMaterial, metrics.totalSales).map(
								(item: any, index: number) => ({
									name: item.material,
									value: item.percent,
									color: [
