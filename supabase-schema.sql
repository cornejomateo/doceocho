...
-- RLS disabled for transactions_box to allow server-side operations
-- In production, consider implementing proper RLS with user context
...
create table public.arca_config (
  ...
  wsfe_service character varying not null,
  company_name character varying not null,
  company_address character varying null,
  is_active boolean not null default true,
  constraint arca_config_pkey primary key (id)
) TABLESPACE pg_default;

-- RLS disabled for arca_config to allow server-side operations
-- In production, consider implementing proper RLS with user context or using service_role for sensitive operations
...
create table public.invoices (
  ...
  cae character varying not null,
  cae_due_date date not null,
  total_amount numeric(10, 2) not null,
  payment_method character varying not null,
  client_name character varying null,
  client_cuit character varying null,
  client_address character varying null,
  pdf_url character varying null,
  qr_code_data text null,
  constraint invoices_pkey primary key (id),
  constraint invoices_transaction_id_fkey foreign key (transaction_id) references public.transactions_box (id) on update CASCADE on delete CASCADE,
  constraint chk_total_amount_non_negative check ((total_amount >= (0)::numeric))
) TABLESPACE pg_default;

-- RLS disabled for invoices to allow server-side operations
-- In production, consider implementing proper RLS with user context